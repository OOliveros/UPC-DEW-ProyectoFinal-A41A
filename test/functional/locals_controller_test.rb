...
class LocalsControllerTest < ActionController::TestCase
  ...
  test "should create local" do
    assert_difference('Local.count') do
      post :create, local: { address: @local.address, district: @local.district, name: @local.name, phone: @local.phone, url: @local.url }
    end
  ...
  test "should update local" do
    put :update, id: @local, local: { address: @local.address, district: @local.district, name: @local.name, phone: @local.phone, url: @local.url }
    assert_redirected_to local_path(assigns(:local))
  end
